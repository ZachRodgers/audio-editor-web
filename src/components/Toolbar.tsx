import { Sun, Moon, Plus, Scissors, Trash2, SkipBack, SkipForward, Play, Pause, Undo2, Redo2, Copy, ClipboardPaste, Files, Keyboard } from "lucide-react";
...
export function Toolbar({ isDark, onToggleTheme }: { isDark: boolean; onToggleTheme: () => void }) {
    ...
    const openClearAll = () => {
        ...
        host.innerHTML = `<div class=\"dialog\"><header class=\"flex items-center gap-2 p-2 border-b\" style=\"border-color: var(--border)\"><div class=\"title\">Clear all clips?</div></header><div class=\"content p-3\">This will remove all clips from all tracks.</div><footer class=\"flex gap-2 justify-end p-2 border-t\" style=\"border-color: var(--border)\"><button id=\"no\" class=\"btn\">No</button><button id=\"yes\" class=\"btn\">Yes</button></footer></div>`;
        document.body.appendChild(host);
        host.querySelector('#no')?.addEventListener('click', () => { document.body.removeChild(host); });
        host.querySelector('#yes')?.addEventListener('click', () => { useStore.setState(s => ({ tracks: s.tracks.map(t => ({ ...t, clips: [] })), selectedClipId: null } as any)); document.body.removeChild(host); });
    };
    ...
    return (
        <>
            <div className="toolbar sticky top-0 z-10 flex items-center gap-3 p-2 border-b" style={{ background: "var(--panel)", borderColor: "var(--border)" }}>
                {/* Left group: AddMedia(+), Duplicate, Copy, Paste, Undo, Redo, Zoom */}
                <div className={`flex items-center gap-2 flex-1 ${keyframeActive ? 'opacity-30 pointer-events-none' : ''}`}>
                    ...
                    <span className="chip text-xs px-2 py-1 rounded-full border" style={{ borderColor: "var(--border)", color: "var(--muted)", background: "var(--panel2)" }}>
                        Zoom <input className="range align-middle" type="range" min={50} max={600} value={pxPerSecond} onChange={(e) => setZoom(parseInt((e.target as HTMLInputElement).value))} />
                    </span>
                </div>
                ...
                <div className="flex items-center gap-2 flex-1 justify-end">
                    <IconButton title="Keyboard" aria-label="Keybinds" onClick={() => document.querySelector('.modal')?.classList.remove('hidden')}><Keyboard className="w-4 h-4" /></IconButton>
